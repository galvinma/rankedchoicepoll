...
it('correctly tallies the first fruit poll', () => {
  ...
  expect(result[result.length-1]['tally']).toEqual({ "apple": 5, "guava": 4 })
  expect(result[result.length-1]['leader']).toEqual(["apple"])
...
it('correctly tallies the second fruit poll', () => {
  ...
  expect(result[result.length-1]['tally']).toEqual({ "apple": 4, "banana": 5 })
  expect(result[result.length-1]['leader']).toEqual(["banana"])
  expect(result[result.length-1]['count']).toBe(9)
});

it('correctly tallies the first car poll', () => {
  let result = iterateTally(car_poll_one, car_poll_items, 0.5)
  expect(result[result.length-1]['tally']).toEqual({"Ford Explorer": 10, "Nissan Rogue": 5, "Nissan Versa": 5})
  expect(result[result.length-1]['leader']).toEqual(["Ford Explorer"])
  expect(result[result.length-1]['count']).toBe(20)
});

it('correctly tallies the first color poll', () => {
  let result = iterateTally(color_poll_one, color_poll_items, 0.5)
  expect(result[result.length-1]['tally']).toEqual({"red": 11, "yellow": 12})
  expect(result[result.length-1]['leader']).toEqual(["yellow"])
  expect(result[result.length-1]['count']).toBe(23)
});
...
it('handles an initial tie, && a leader from the first round of voting', () => {
  ...
  expect(result[result.length-1]['tally']).toEqual({ "apple": 3, "banana": 3, "guava": 3 })
  expect(result[result.length-1]['leader']).toEqual(["apple"])
  expect(result[result.length-1]['count']).toBe(9)
});
...
it('handles an initial tie, && a tie in the first round of voting', () => {
  ...
  expect(result[result.length-1]['tally']).toEqual({"apple": 3, "peach": 3, "strawberry": 3})
  expect(result[result.length-1]['leader']).toEqual(["apple", "strawberry", "peach"])
  expect(result[result.length-1]['count']).toBe(9)
});
